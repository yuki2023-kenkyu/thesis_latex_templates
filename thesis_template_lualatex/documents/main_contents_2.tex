...
この論文テンプレートは，熊本大学 岡島寛先生らの卒論・修論テンプレートを参考に作成しています\cite{卒論修論テンプレート}．
	...
	論文中の数式を引用したい場合は，引用したい数式にラベルを付け，引用したい箇所で「\textbackslash cref\{eq:作成したラベル\}」と入力すれば\zcref{eq:Ricci_tensor}のように引用が可能です．数式ラベル内にeq:とつけているのは数式をエディタの検索機能や補完機能を利用しやすくするためです．
